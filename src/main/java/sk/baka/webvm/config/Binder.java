		...
		try {
			for (final Field field : bean.getClass().getFields()) {
				...
				if (beanToMap) {
					...
					if (validate) {
						result.putAll(checkValidity(fieldValue, annotation));
					}
					if (fieldValue != null) {
						properties.put(annotation.key(), fieldValue.toString());
					} else {
						properties.remove(annotation.key());
					}
				} else {
					final Class<?> fieldClass = primitiveToClass(field.getType());
					final String value = properties.getProperty(annotation.key());
					if (value == null) {
						continue;
					}
					...
					try {
						...
						result.put(annotation.key(), "Failed to parse " + value + ": " + ex.toString());
						continue;
					}
					if (validate) {
						result.putAll(checkValidity(fieldValue, annotation));
